...
class c4_LifecycleHooks extends LifecycleHooksBase {
    ...
    void no_subscription_no_gains() {
        ...
        final Flux<Integer> temperatureFlux = room_temperature_service().doOnSubscribe(s -> hooksTriggered.add("subscribe"));
    ...
    @Test
    void be_there_early() {
        final CopyOnWriteArrayList<String> hooksTriggered = new CopyOnWriteArrayList<>();

        final Flux<Integer> temperatureFlux = room_temperature_service().doFirst(() -> hooksTriggered.add("before subscribe"));

        StepVerifier.create(temperatureFlux.take(5).doOnSubscribe(s -> hooksTriggered.add("subscribe"))).expectNextCount(5).verifyComplete();
    ...
    void atomic_counter() {
        ...
        final Flux<Integer> temperatureFlux = room_temperature_service().doOnNext(i -> {
    ...
    void successfully_executed() {
        ...
        final Flux<Integer> temperatureFlux = room_temperature_service().doOnComplete(() -> completed.set(true));
    ...
    void need_to_cancel() {
        ...
        final Flux<Integer> temperatureFlux = room_temperature_service().doOnCancel(() -> canceled.set(true));
    ...
    @Test
    void terminator() {
        final AtomicInteger hooksTriggeredCounter = new AtomicInteger(0);

        final Flux<Integer> temperatureFlux = room_temperature_service().doOnTerminate(() -> hooksTriggeredCounter.incrementAndGet());

        StepVerifier.create(temperatureFlux.take(0)).expectNextCount(0).verifyComplete();

        StepVerifier.create(temperatureFlux.skip(20)).expectNextCount(0).verifyComplete();

        StepVerifier.create(temperatureFlux.skip(20).concatWith(Flux.error(new RuntimeException("oops")))).expectError().verify();
    ...
    void ordering_is_important() {
        ...
        Mono<Boolean> just = Mono.just(true).doFirst(() -> sideEffects.add("three")).doFirst(() -> sideEffects.add("two")).doFirst(() -> sideEffects.add("one"));
