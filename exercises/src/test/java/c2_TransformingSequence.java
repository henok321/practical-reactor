...
class c2_TransformingSequence extends TransformingSequenceBase {
    ...
    void transforming_sequence() {
        final Flux<Integer> numbersFlux = numerical_service()
                .map(i -> i + 1);
    ...
    @Test
    void cast() {
        final Flux<String> numbersFlux = object_service().cast(String.class);
    ...
    @Test
    void maybe() {
        final Mono<String> result = maybe_service().defaultIfEmpty("no results");
    ...
    @Test
    void sequence_sum() {
        final Mono<Integer> sum = numerical_service().reduce(Integer::sum);
    ...
    void sum_each_successive() {
        final Flux<Integer> sumEach = numerical_service().scan(Integer::sum);
    ...
    void sequence_starts_with_zero() {
        ...
        final Flux<Integer> result = numerical_service().startWith(0);
